/**
 * tsdown build for dsh-side-chat: the host-half lib (lib/index.js, ESM node)
 * plus the two browser client bundles (lib/client.js for the official profile
 * channel, lib/client-registry.js for the plugin-registry channel) — one per
 ...
 * rejects any other @deepseek-ai value import — cross-plugin collaboration
 * goes through cordis services, never value imports. CSS Modules compile to
 * hashed class maps and inject <style data-plugin> tags at factory execution.
 */
...
function purityGatePlugin(): BuildPlugin {
  return {
    ...
    resolveId(source: string) {
      ...
      throw new Error(
        `client bundle purity: "${source}" is not a platform module and not an inline-safe wire layer — cross-plugin value imports are forbidden`,
      )
    },
  }
}
