...
- **AI 回复带回主会话。** 侧边聊天里的每条 AI 回复都能带回当前主会话输入框：可鼠标选中
  部分内容后带回，或一键带回整段；两种方式都可选择 **「直接带回」**（原文）或
  **「摘要后带回」**（先用侧边聊天继承的模型生成摘要再带回）。带回内容只写入主会话输入框
  草稿，不会立即发送。
...
侧边聊天里的 AI 回复可以带回当前主会话的输入框（只写入草稿，**不会立即发送**）：
...
3. 带回的内容会追加到主会话输入框草稿末尾（已有内容时另起一段），由你确认、编辑后再发送。
